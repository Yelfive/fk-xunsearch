...
class XS extends XSComponent
{
    ...
    /**
     * Set config, based on [[overwrite]].
     * @param array $config
     * @param bool $overwrite
     */
    public function setConfigs(array $config, bool $overwrite)
    {
        if ($overwrite === true) {
            $array1 = $this->_config;
            $array2 = $config;
        ...
        $this->_config = array_merge($array1, $array2);
    }
     ...
     * @return mixed 转换后的数据
     * @throws XSException 如果没有合适的转换函数抛出异常
     */
    ...
    private function parseIniData($data)
    {
        ...
        if (!$ret['server.search'] && Yii::$app->get('xunsearch') && Yii::$app->get('xunsearch')) {
     ...
     *
     * Don't know where to restore, so just add Yii check
     * And XS::errorHandler didn't consider the errors triggered from outside the xunsearch lib
     *
     */
    public static function errorHandler($code, $message, $file, $line)
    {
        if (class_exists('Yii', false)) { // don't overwrite Yii error handler
            Yii::$app->errorHandler->handleError($code, $message, $file, $line);
        } else if ((ini_get('error_reporting')) && !strncmp($file, XS_LIB_ROOT, strlen(XS_LIB_ROOT))) {
            throw new XSErrorException($code, $message, $file, $line);
        }
    }
}
...
set_error_handler(['XS', 'errorHandler']);
